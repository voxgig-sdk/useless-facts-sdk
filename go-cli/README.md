...
boru-driven command-line client **and** interactive REPL for the UselessFacts
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/useless-facts-cli)
make build

# 2. See usage (words, entities, env vars)
./useless-facts-cli --help

# 3. Provide credentials once, via the environment
export USELESS_FACTS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./useless-facts-cli load 1 random            # {id:1} shorthand
./useless-facts-cli load '{id:1}' random       # explicit match map

# 5. Override the API base URL for a single call
USELESS_FACTS_BASE=https://api.example.com ./useless-facts-cli load 1 random

# 6. No arguments -> interactive REPL
./useless-facts-cli
useless-facts> load 1 random
useless-facts> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/useless-facts-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/useless-facts-cli load 1 random
   ```
...
```sh
./useless-facts-cli load 1 random          # scalar shorthand for {id:1}
./useless-facts-cli load '{id:1}' random     # explicit match map
```
...
./useless-facts-cli load 1 random
```
...
Run with no arguments to open a REPL (prompt `useless-facts>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/useless-facts-cli`. |
...
(`./useless-facts-cli <expr>`) and interactively (the REPL), and expressions compose the
