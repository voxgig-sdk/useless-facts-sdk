...
// UselessFacts SDK retry feature
...
class UselessFactsRetryFeature extends UselessFactsBaseFeature
{
    ...
    public function init(UselessFactsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (UselessFactsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(UselessFactsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(UselessFactsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
