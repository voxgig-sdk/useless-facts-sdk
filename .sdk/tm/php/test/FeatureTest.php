...
class FtCtrl extends UselessFactsControl
{
...
class FtClient
{
    ...
    private UselessFactsUtility $utility;

    public function __construct(UselessFactsUtility $utility, array $options)
    {
    ...
    public function get_utility(): UselessFactsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UselessFactsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public UselessFactsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (UselessFactsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UselessFactsConfig::make_feature($name);
    ...
    private static function buildUrl(UselessFactsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UselessFactsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UselessFactsResponse($fetched);
        }
    ...
    private function fail(UselessFactsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UselessFactsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
