...
module UselessFactsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UselessFactsResponse.new({})
    ...
    if fetch_err
      ...
      response = UselessFactsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UselessFactsResponse.new(fetched)
    else
