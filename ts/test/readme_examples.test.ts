...
//     (\`new UselessFactsSDK(...)\` / \`UselessFactsSDK.test(...)\` become
//     \`UselessFactsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'UselessFactsSDK'
...
// rewrite real client construction (\`new UselessFactsSDK(...)\` and any
// \`UselessFactsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UselessFactsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UselessFactsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
