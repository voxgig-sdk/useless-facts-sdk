...
[MCP](https://modelcontextprotocol.io) server exposing the UselessFacts SDK as
two agent tools — `useless-facts_list` and `useless-facts_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/useless-facts-mcp)
make build

# 2. Provide credentials via the environment
export USELESS_FACTS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user useless-facts \
  -- /absolute/path/to/useless-facts-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./useless-facts-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// useless-facts_list: first page of records
{ "entity": "random" }
{ "entity": "random", "query": { } }

// useless-facts_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/useless-facts-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user useless-facts \
     -- "$PWD"/dist/*/useless-facts-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `useless-facts_list` and `useless-facts_load` tools now appear
   in new sessions. Ask the agent to *"list random using useless-facts"*
   and it calls `useless-facts_list` with `{"entity":"random"}`.
...
```sh
export USELESS_FACTS_APIKEY=sk_live_xxx            # API key
export USELESS_FACTS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `useless-facts_list` tool
...
### Call the `useless-facts_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `useless-facts_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `useless-facts_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `USELESS_FACTS_APIKEY` | API key sent with every request. |
| `USELESS_FACTS_BASE` | Optional override of the API base URL. |
...
```sh
./useless-facts-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"useless-facts_load","arguments":{"entity":"random","query":{"id":1}}}}'
```
