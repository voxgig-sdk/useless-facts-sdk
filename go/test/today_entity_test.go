...
func TestTodayEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		todayRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.today")))
		...
		todayRef01DataDt0LoadResult := core.ToMapAny(entityData(todayRef01DataDt0Loaded))
...
func todayBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("USELESS_FACTS_TEST_TODAY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["USELESS_FACTS_TEST_TODAY_ENTID"])
