...
func TestRandomEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set USELESSFACTS_TEST_RANDOM_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func randomBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("USELESSFACTS_TEST_RANDOM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["USELESSFACTS_TEST_RANDOM_ENTID"])
