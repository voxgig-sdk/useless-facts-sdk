...
import (
	...
	omni "github.com/voxgig-sdk/useless-facts-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UselessFactsError); is {
				out := map[string]any{
					"name":    "UselessFactsError",
...
func omniCtx(arg any, client *sdk.UselessFactsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UselessFactsSDK); is {
			client = live
