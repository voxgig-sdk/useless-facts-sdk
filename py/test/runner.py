# UselessFacts SDK test runner

from __future__ import annotations
import os
import json

from uselessfacts_sdk.utility.voxgig_struct import voxgig_struct as vs


class UselessFactsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            UselessFactsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = UselessFactsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = UselessFactsTestRunner.getenv("USELESS_FACTS_TEST_LIVE")
        override = UselessFactsTestRunner.getenv("USELESS_FACTS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = UselessFactsTestRunner.getenv(key)
        ...
        explain = UselessFactsTestRunner.getenv("USELESS_FACTS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if UselessFactsTestRunner._test_control is not None:
            return UselessFactsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                UselessFactsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            UselessFactsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = UselessFactsTestRunner.load_test_control()
...
def load_env_local():
    UselessFactsTestRunner.load_env_local()


def env_override(m):
    return UselessFactsTestRunner.env_override(m)


def entity_data(v):
    return UselessFactsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return UselessFactsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return UselessFactsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return UselessFactsTestRunner.load_test_control()


def live_client_options():
    return UselessFactsTestRunner.live_client_options()


def live_delay_ms():
    return UselessFactsTestRunner.live_delay_ms()
