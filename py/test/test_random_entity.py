    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set USELESSFACTS_TEST_RANDOM_ENTID JSON to run live")
        client = setup["client"]
...
def _random_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("USELESSFACTS_TEST_RANDOM_ENTID"))
    ...
    _live = env.get("USELESSFACTS_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("USELESSFACTS_TEST_EXPLAIN") == "TRUE",
        "live": _live,
