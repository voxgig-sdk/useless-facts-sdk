...
require_relative 'UselessFacts_types'
...
class UselessFactsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = UselessFactsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = UselessFactsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, UselessFactsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    UselessFactsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = UselessFactsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = UselessFactsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = UselessFactsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => UselessFactsError.new(
        "#{op}_allow",
        "UselessFactsSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = UselessFactsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = UselessFactsError.new(
        "graphql_error", "UselessFactsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = UselessFactsSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
