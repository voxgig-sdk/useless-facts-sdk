# UselessFacts SDK Random entity
...
class RandomEntity
  ...
  def data_set(args)
    if args
      @_data = UselessFactsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = UselessFactsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Random, Hash] the loaded Random; raises UselessFactsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = UselessFactsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
